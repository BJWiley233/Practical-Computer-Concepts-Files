				...
				foreach my $prop (@$property) {
					if ($prop->{value} eq 'X-ray') {
						...
						my $prop_ordered = Hash::Ordered->new(type  => "method",
															  value => "X-ray");
						$proteins{$uniprot_ids[0]}{$val->{id}}{'property'} = [$prop_ordered]; 
...
print $proteins{$uniprot_ids[0]}{'2LGC'}{'property'}[0]->get('type') . "\n";
print $proteins{$uniprot_ids[0]}{'2LGC'}{'property'}[0]->get('value') . "\n";
	...
	foreach my $pdb (keys %{ $proteins{$uniprot} }) {
		...
		if ($proteins{$uniprot}{$pdb}{'property'}) {
			...
			foreach my $prop (@{ $proteins{$uniprot}{$pdb}{'property'} }) {
				...
				## Ordered Hash requires iterator, same as Java :0. https://metacpan.org/pod/Hash::Ordered#iterator
...
#binmode $out;
#$doc->toFH($out);
$doc->toFile('uniprot_pdb_out.xml', 1);
