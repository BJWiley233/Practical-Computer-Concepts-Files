...
''' extra test set 2
xpoints = np.random.choice(range(-10,21), 15, replace=False)
ypoints = np.random.choice(range(-10,21), 15)
...
'''
update to Jade's calculations for n = 7
# for a = 0 we have 6 calculations (n-1) calculations
    ## for the rest [1..n-2] we always have (n-2) calculations because each time we have to choose from 
    ## we chose from a=min calculations and then make n-a-2 post calculations gives us a+n-a-2 = n-2 calculations
    ## we also make total [0 + 1 + 2 + ... n-3] comparisons = 1/2n^2-5/2n+3 by Summation A5
# for a = 1 we have 1 min calculations and (1-1 comparisons) + 4 post calculations (1,4),(1,5)..(1,n-1) we might not need this last one?
# for a = 2 we have 2 min calculations and (2-1 comparisons) + 3 post calculations (2,4),(2,5),(2,n-1) we might not need this last one?
# for a = 3 we have 3 min calculations and (3-1 comparisons) + 2 post calculations (3,5), (3,n-1) we might not need this last one?
# for a = 4 we have 4 min calculations and (4-1 comparisons) + 1 post calculations (4,6) we might not need this last one?
# for a = i we have i min calculations and (i-1 comparisons) + n-i-2 post calculations; maybe it's n-i-3 post calculations
                                                                                        as I don't think we use the last one??
# ...
# for a = n-2 we have (n-2) min calculations (n-3 comparisons)
'''
# total run time is (n-1) + (n-2)(n-2) + 1/2n^2-5/2n+3 = O(3/2n^2-11/2n+6) = O(n^2)
