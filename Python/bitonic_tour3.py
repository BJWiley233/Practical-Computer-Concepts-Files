...
points_dict = {x:i for x, i  in enumerate(points_sorted)}
points_dict1 = {x+1:i for x, i  in enumerate(points_sorted)}
...
for i in range(0, n):
    for j in range(i+1, n):
        distance_matrix[i,j] = round(distance(points_sorted[i], points_sorted[j]), 3)
...
for i in range(0,n-1):
    for j in range(i+1,n):
        ...
        if j == n-1 and i < j-1:
            continue
        print(i+1, j+1)
        subtour(i, j, tour_dict, distance_matrix)
...
def find_full_tour(P):
    ...
    return final_tour_unordered, final_distance

# need to add last edge of tour
tt, td = find_full_tour(points_list)
'''
...
final_tour_unordered_arrow = [i.split("->") for i in final_tour_unordered_comma]
final_tour_unordered_int = [[int(i) for i in x] for x in final_tour_unordered_arrow]
final_tour_unordered_int1 = [[int(i)+1 for i in x] for x in final_tour_unordered_arrow]
print("1 index unordered:")
for i in final_tour_unordered_int1:
    print(i[0], end = '')
    print('->', end = '')
    print(i[1], end = '')
    print(',', end = '')
...
final = [final_tour_unordered_int.pop(0)]
final1 = [final_tour_unordered_int1.pop(0)]
...
for edge in final_tour_unordered_int1:
        if final1[-1][1] == edge[0]:
            final1.append(edge)
j = 0
...
final_tour_unordered_int1 = [x for x in final_tour_unordered_int1 if x not in final1]
# then we reverse since we are at the end
final_tour_unordered_int.reverse()
final_tour_unordered_int1.reverse()
...
### FIXME END
while final_tour_unordered_int1:
    final1.append(final_tour_unordered_int1.pop(0)) 
...
final_string_OMG1 = [[str(i) for i in x] for x in final1]
final_tour1 = ""
for i, edge in enumerate(final_string_OMG1):
    if i == 0:
        final_tour1 = final_tour1 + edge[0] + "->" + edge[1]
    else:
        final_tour1 = final_tour1 + "->" + edge[1]        
# YAY!!!
print("Maybe the messiest Bitonic Tour Code EVER")
print("Tour is {} and shortest distance is {}".format(final_tour, final_tour_distance))
print("Tour is {} and shortest distance is {}".format(final_tour1, final_tour_distance))
...
for n, p in pos1.items():
    G1.node[n]['pos'] = p
nx.draw_networkx(G1, pos1, node_size=600, )
nx.draw_networkx_edges(G1, pos1, edgelist=edgelist1, width=3)
plt.show()
#points_list = [[7,5], [2,3], [1,0], [0,6], [5,4], [6,1], [8,2]]  
print("[", end = '')
