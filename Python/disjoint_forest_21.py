...
class Graph():
    ...
    def make_sets(self, set_list):
        for i, set_ in enumerate(set_list):
            self.sets[i] = None
            nodes = [Node(s) for s in set_]
            for j in range(0, len(nodes)-1):
                self.link_init(i, nodes[j], nodes[j + 1])  
    
    def find_set_init(self, x):
    ...
    def link_init(self, set_j, x, y):
        if(x != y):
            if self.sets[set_j].rank > self.sets[set_j].rank:
                self.sets[set_j].p = self.sets[set_j].value
            else:
                self.sets[set_j].p = self.sets[set_j].value
                if self.sets[set_j].rank == self.sets[set_j].rank:
                    self.sets[set_j].rank += 1
            
            temp = self.sets[set_j].back
            self.sets[set_j].back = self.sets[set_j].back
            self.sets[set_j].back = temp            
