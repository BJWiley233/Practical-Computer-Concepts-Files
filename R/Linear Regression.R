...
g <- g + scale_color_gradient(low = "light blue", high = "white")
g
...
g <- ggplot(filter(freqData, freq > 0), aes(x = parent, y = child)) + 
        xlab(expression(paste("parent ", inches^{2})))
g <- g  + scale_size(range = c(2, 20), guide = "none" )
...
mse <- mean((y - beta * x)^2)
g <- g + ggtitle(paste("beta = ", beta, ", mse = ", round(mse, 3), sep = ""))
g
}
manipulate(myPlot(beta), beta = slider(.6,1.2, step = 0.02))




if mydata$prog[mydata$=]
