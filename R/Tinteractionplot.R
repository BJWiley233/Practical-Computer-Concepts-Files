Tinteraction.plot <- function (x.factor, trace.factor, response, fun = mean, type = c("l", 
                               ...
                               xtick = FALSE, xaxt = par("xaxt"), axes = TRUE, ...) 
{
        ...
        if (legend) {
                ...
                text(xleg, ylim[2L] - 0.01 * yrng, paste("  ", trace.label), 
                     adj = 0)
                if (!fixed) {
                ...
                legend(xleg, yleg, legend = ylabs, col = col, pch = if (type %in% 
                        ...
                        pch, lty = if (type %in% c("l", "b")) 
                                lty, bty = leg.bty, bg = leg.bg, cex = .56, pt.cex = 10, ncol = 2)
        }
        invisible()
}
